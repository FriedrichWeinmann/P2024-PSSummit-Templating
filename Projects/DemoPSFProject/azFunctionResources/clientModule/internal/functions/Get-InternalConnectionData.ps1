...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'DemoPSFProject.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'DemoPSFProject.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'DemoPSFProject.Client.ProtectedToken'
