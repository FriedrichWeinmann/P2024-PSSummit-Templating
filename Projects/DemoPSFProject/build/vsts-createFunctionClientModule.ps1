...
$parentModule = 'DemoPSFProject'
if (-not $ModuleName) { $ModuleName = 'DemoPSFProject.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\DemoPSFProject'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\DemoPSFProject.psd1"
$prereqHash = @{
